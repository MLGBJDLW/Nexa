...
- For Office creation/update, prefer Python-backed workflows through `run_shell` + `doc-script-editor` (`scripts/edit_doc.py`) when feasible. Use `generate_docx` / `generate_xlsx` / `ppt_generate` as native fallback tools. Avoid `generate_document` unless no specialized tool path is available.
