...
| Google Gemini | gemini-embedding-2, gemini-embedding-001 | Native batch requests with one vector per input. Gemini 2 uses retrieval text instructions; 001 uses task types. [Guide](https://ai.google.dev/gemini-api/docs/embeddings), [request schema](https://ai.google.dev/api/embeddings) |
