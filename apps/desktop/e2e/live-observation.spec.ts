...
test.beforeEach(async ({ page, context }) => {
  ...
  await page.addInitScript(() => {
    ...
    let seq=1; let lid=1; let delayed=false; let holdReady=false; let failStop=false; let release:(()=>void)|undefined;
    ...
    const invoke=async (cmd:string,args:Record<string,unknown>={})=>{
      switch(cmd){
        ...
        case 'stop_live_cmd':state.stopped++;if(failStop){failStop=false;throw new Error('Stop response lost');}state.snapshot.phase='stopped';return structuredClone(state.snapshot);
...
for (const recovery of ['retry', 'terminal event'] as const) {
  test(`retains a failed stop for ${recovery} recovery`,async({page})=>{
    await chooseQwen(page);
    await page.getByRole('button',{name:'Start Live',exact:true}).click();
    await expect.poll(async()=>(await state(page)).audio).toBeGreaterThan(1);
    await page.evaluate(()=>(window as unknown as {__failNextStop:()=>void}).__failNextStop());
    await page.getByRole('button',{name:'Stop capture'}).click();
    await expect(page.getByRole('alert')).toContainText('Stop response lost');
    await expect.poll(async()=>(await state(page)).tracks.every(track=>track==='ended')).toBe(true);
    if (recovery === 'retry') {
      await page.getByRole('button',{name:'Stop capture'}).click();
      await expect.poll(async()=>(await state(page)).stopped).toBe(2);
    } else {
