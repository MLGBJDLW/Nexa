...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    history.replaceState(
      { usr: { initialMessage: 'Show me a long streaming answer.' }, key: 'e2e-long-stream', idx: 0 },
      '',
      '/chat',
    );
    ...
    const invoke = async (cmd: string, args: Record<string, unknown> = {}) => {
      switch (cmd) {
        ...
        case 'create_conversation_cmd': {
          const id = 'conv-long-stream';
          const conversation: Conversation = {
            id,
            title: 'Long Stream Test',
...
test('keeps long streaming markdown advancing before the stream finishes', async ({ page }) => {
