...
import { Activity, CircleStop, FileText, Loader2, MessageCircle, Mic, Radio } from 'lucide-react';
...
export function LiveWorkspace({ transport, onSendToChat }: { transport: LiveTransport; onSendToChat: (text: string) => void }) {
  ...
  const desktopWindows = useDesktopWindows(Boolean(transport.nativeScreenCapture));
  const [connections, setConnections] = useState<LiveConnection[]>([]);
  const [connectionId, setConnectionId] = useState('');
  const [summaryId, setSummaryId] = useState('');
  const [observerModel, setObserverModel] = useState<TurnModelSelection | null>(null);
  const [summaryModel, setSummaryModel] = useState<TurnModelSelection | null>(null);
  ...
  async function summarize() {
    ...
    try { const record = await transport.summarize(id, summaryId, summaryModel); if (mounted.current) setSummary(record.summary || ''); }
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[320px_minmax(0,1fr)]">
      <section className="space-y-4 rounded-xl border border-border bg-surface-1/70 p-5" aria-label={t('live.setup')}>
        ...
        <fieldset disabled={live.active || loading} className="space-y-4">
          <label className="block space-y-1.5 text-xs font-medium text-text-secondary"><span>{t('live.observer')}</span><select className={field} value={connectionId} onChange={event => { setConnectionId(event.target.value); setObserverModel(null); setProtocol(''); }}><option value="" disabled>{loading ? t('live.connecting') : t('live.selectConnection')}</option>{connections.map(item => <option key={item.id} value={item.id}>{item.name} · {item.model}</option>)}</select></label>
          {!protocol && transport.models && <ConnectionModelPicker connectionId={connectionId} defaultModel={selected?.model || ''} value={observerModel} onChange={setObserverModel} load={transport.models} label={t('live.observationModel')} disabled={live.active || loading} />}
          ...
          {protocol && (transport.models ? <ConnectionModelPicker connectionId={connectionId} defaultModel={models[protocol]} value={{ model, reasoningEnabled:null, reasoningEffort:null, thinkingBudget:null }} onChange={selection => setModel(selection?.model || models[protocol])} load={transport.models} label={t('live.model')} disabled={live.active || loading} /> : <label className="block space-y-1.5 text-xs font-medium text-text-secondary"><span>{t('live.model')}</span><input className={field} value={model} maxLength={160} onChange={event => setModel(event.target.value)} /></label>)}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block text-xs text-text-secondary"><span className="sr-only">{t('live.visualInput')}</span><select aria-label={t('live.visualInput')} className={field} value={source} onChange={event => setSource(event.target.value as LiveVideoSource)}><option value="none">{t('live.none')}</option><option value="camera">{t('live.camera')}</option>{monitors.map((monitor, index) => <option key={monitor.id} value={`monitor:${monitor.id}`}>{t('live.screen')} {index + 1} · {monitor.width} × {monitor.height}</option>)}{desktopWindows.windows.map(window => <option key={window.id} value={`window:${window.id}`}>{window.title} · {window.appName}</option>)}{(!transport.nativeScreenCapture || desktopWindows.supported === false) && typeof navigator.mediaDevices?.getDisplayMedia === 'function' && <option value="screen">{t('chat.shareWindow')}</option>}</select></label>
          </div>
      ...
      <div className="min-w-0 space-y-5">
        ...
        <LiveRecordPanels snapshot={live.snapshot} summary={summary} summarizing={summarizing} controls={<>
<div className="flex flex-wrap items-end gap-3"><label className="min-w-40 flex-1 space-y-1.5 text-xs font-medium text-text-secondary"><span>{t('live.summaryModel')}</span><select className={field} value={summaryId} disabled={summarizing} onChange={event => { setSummaryId(event.target.value); setSummaryModel(null); }}>{connections.map(item => <option key={item.id} value={item.id}>{item.name} · {item.model}</option>)}</select></label><button className={button} disabled={live.active || !live.snapshot?.entries.length || !summaryId || summarizing} onClick={() => void summarize()}>{summarizing && <Loader2 size={15} className="animate-spin" />}{t('live.summarize')}</button><button className={button} disabled={live.active || !evidence || summarizing} onClick={() => onSendToChat(`${t('live.handoff')}\n\n${evidence}`)}><MessageCircle size={15} />{t('live.sendToChat')}</button></div>
          {transport.models && <ConnectionModelPicker connectionId={summaryId} defaultModel={connections.find(item => item.id === summaryId)?.model || ''} value={summaryModel} onChange={setSummaryModel} load={transport.models} label={t('live.summaryModelChoice')} disabled={summarizing} />}
        </>} />
