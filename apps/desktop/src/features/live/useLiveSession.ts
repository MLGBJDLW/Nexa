  ...
  const start = useCallback(async (request: StartLiveRequest, source: LiveVideoSource) => {
    ...
    try {
      ...
      if (request.microphone) {
        const queue = new LiveAudioQueue(Math.round(ready.sampleRate / 10) * 2, bytes => transport.audio(opened.id, bytes), fail, transport.audioWindow, transport.recoverAudio);
